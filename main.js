...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let x0, v0, A, G, Wn, Y, Wd, alpha, G1, G2, beta, regimen, A1, A2, Yc, Ac, regimenc, betac, alphac, G1c, G2c, A1c, A2c, Wdc, Gc, Wnc, raizdelta, K, B, m = 0.5, ultimoGc, ultimolWf, bcambio = 0;
...
// Ajustes del botón aplicar cambios.
    ...
    aplicarCambios.addEventListener('click', function () { 
        ...
        if (!isNaN(valorWf) && !isNaN(valorAf0) && !isNaN(nuevoFactorAmortiguamiento) && !isNaN(nuevaFrecuenciaAngular)
            ...
            if (idAnimacion) {
                ...
                x0 = calcularXNueva(tc, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf);
                v0 = calcularVNueva(tc, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf);
                bcambio = 1, cam=true;
    ...
    function calcularXNueva(tiempo, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf) {
        xNUEVA = 0;
        if (tiempo < 0.020) {
        ...
        if (regimen == 0) {//XNueva para el caso subamortiguado
            ...
            xNUEVA = (A1 * Math.exp(-G1 * (tiempo))) + (A2 * Math.exp(-G2 * tiempo));
        }
    ...
    function calcularVNueva(tiempo, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf) {
        vNUEVA;
        ...
        if (regimen == 0) {//VNueva para el caso subamortiguado
            ...
            vNUEVA = -A1 * G1 * Math.exp(-G1 * tiempo) - A2 * G2 * Math.exp(-G2 * (tiempo));
    ...
    function calcularparámetrosInternos(tc, xc, vc, Wn, G, bcambio, historialGc, m, Af0, Wf, historialWf,cam) {
        ...
        Ac = 0, Yc = 0, betac = 0, alphac = 0, G1c = 0, G2c = 0, A1c = 0, A2c = 0, regimenc = 0, Wdc = 0, Afc = 0, Yfc = 0, Y1c = 0;
        ...
        if (G < Wn) {   //Calculo para el caso subamortiguado.
            ...
            A1 = ((vcf + G2 * xcf) / (G2 - G1)) * Math.exp(G1 * (tc));
            A2 = ((vcf + G1 * xcf) / (G1 - G2)) * Math.exp(G2 * (tc));
        ...
        if(cam==true){
        X2 = calcularXNueva(tc, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf);}
        ...
        function actualizar() {
            ...
            if (tiempoDesdeUltimaActualizacion >= tiempoEntreActualizaciones) {//Encargado de la actualización de la función actualizar.
                ...
                if (tiempo > tc) {//Calculo con parámetros nuevos.
                    xNUEVA = calcularXNueva(tiempo, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf);
                    vNUEVA = calcularVNueva(tiempo, A, Y, G, Wd, regimen, alpha, beta, A1, A2, G1, G2, Yf, Y1, Af, Wf);
                    fuerza = calcularfuerza(tiempo,Yf, Af, Wf);

                }
                else {//Calculo de parámetros, para hacer la transición del cambio.
                    xNUEVA = calcularXNueva(tiempo, Ac, Yc, ultimoGc, Wdc, regimenc, alphac, betac, A1c, A2c, G1c, G2c, Yfc, Y1c, Afc, ultimolWf);
                    vNUEVA = calcularVNueva(tiempo, Ac, Yc, ultimoGc, Wdc, regimenc, alphac, betac, A1c, A2c, G1c, G2c, Yfc, Y1c, Afc, ultimolWf);
